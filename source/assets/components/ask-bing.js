// ask-bing.js（ESM）
// 元件用法：
// 網頁載入此元件 <script type="module" src="/assets/components/ask-bing.js"></script>
// 在頁面任意位置放此標籤 <ask-bing q="你的問題"></ask-bing>

class AskBing extends HTMLElement {
  ...
  constructor() {
    ...
    // 監看 slot 內容變更（<ask-bing>內容</ask-bing>）
  ...
  syncUI() {
    ...
    // label 規則：有夾帶內容則顯示 "Ask Bing: 內容"；沒有則呈現 "Ask Bing"
    this._label.textContent = extra ? `Ask Bing : ${extra}` : "Ask Bing";

    // hint 規則：維持顯示 q
    this._btn.title = this.q;
    this._btn.setAttribute("aria-label", extra ? `Ask Bing: ${extra}` : "Ask Bing");
  }

  open() {
    const url = "https://www.bing.com/copilotsearch/?q=" + encodeURIComponent(this.q);
    window.open(url, "_blank", "noopener,noreferrer");
  }
}

customElements.define("ask-bing", AskBing);
